...
# src offsets : de x = 0 à ny_sabN -1 (bloc pole Nord), concaténé avec bloc Sud (de  x = ny - ny_sabS à ny -1  

for i in range(num_links):
    if (i < nx * ny_sabN ): 
        src_address_var[i] = i+1 
    else:
        src_address_var[i] = nx * (ny - ny_sabS - ny_sabN ) + i+1
        if (i == nx*ny_sabN): print(" src_address[nx * ny_sabN] = ",src_address_var[i])
# dst offsets : de x = 0 à ny_SabN-1 (bloc Nord), attention à ligne de 0 de séparation
#           ; puis x = ny_SabN+1 à dst_grid_size (bloc sud)
#dst_address_N = np.linspace(0,nx * ny_sabN-1,nx * ny_sabN) 

#dst_address_S = np.linspace (nx * (ny_sab - (ny_sabS +1)) , nx * ny_sab - 1, nx * ny_sabS)

for i in range(num_links):
    if (i < nx * ny_sabN ):
        dst_address_var[i] = i+1
    else:
        dst_address_var[i] = nx * (ny_sab - ny_sabS - ny_sabN) + i+1 
...
print(" Fichier NetCDF 'rMp_NEMO_to_SAB.nc' créé avec succès.")
