...
CONTAINS
   ...
   SUBROUTINE turb_coare3p6( kt, zt, zu, T_s, t_zt, q_s, q_zt, U_zu, l_use_cs, l_use_wl, &
      ...
      DO jit = 1, nbit
         ...
         ztmp1 = ( znu_a / (z0*u_star) )**0.6_wp     ! COARE3.6-specific! (1./Re_r)^0.72 (Re_r: roughness Reynolds number) COARE3.6-specific!
         z0t   = MIN( 1.15E-4_wp , 5.5E-5_wp*ztmp1 )   ! COARE3.6-specific!
         z0t   = MIN( MAX(ABS(z0t), 1.E-9) , 1._wp )                      ! (prevents FPE from stupid values from masked region later on)
   ...
   FUNCTION charn_coare3p6( pwnd )
      ...
      !
      REAL(wp), PARAMETER :: charn0_max = 0.0317  !: value above which the Charnock parameter levels off for winds > 18 m/s
      !!-------------------------------------------------------------------
      charn_coare3p6 = MAX( MIN( 0.0016_wp*pwnd - 0.0035_wp , charn0_max) , 0._wp )
      !!
