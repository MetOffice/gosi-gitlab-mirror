...
!
   ...
   SUBROUTINE sbc_wave_init
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '      Stokes drift test with constant wave fields ln_wave_test = ', ln_wave_test
      ENDIF

      !                                ! option check
      IF( .NOT.( ln_cdgw .OR. ln_sdw .OR. ln_tauoc .OR. ln_stcor .OR. ln_charn) )   &
         &     CALL ctl_warn( 'Wave coupling/forcing activated but ln_cdgw=F, ln_sdw=F, ln_tauoc=F, ln_stcor=F')
      ...
      IF( ln_sdw ) THEN                      ! Stokes drift
         ...
         IF( ln_zdfswm .AND. .NOT. cpl_wnum ) THEN
            ...
            CALL fld_fill( sf_wnum, (/ sn_wnum /), cn_dir, 'sbc_wave_init', 'Wave module', 'namsbc_wave' )
         ENDIF
         !
      ENDIF
      !
