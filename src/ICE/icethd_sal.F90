         ...
         DO ji = 1, npti
            ...
            IF( h_i_1d(ji) > rn_sal_himin ) THEN
               ...
               IF( ln_drainage .OR. ln_flushing ) THEN
                  zsal = s_i_1d(ji)
                  ! update salinity
                  s_i_1d(ji) = MIN( MAX( rn_simin, s_i_1d(ji) ), rn_sinew*sss_1d(ji) )
                  ! salt flux
                  sfx_res_1d(ji) = sfx_res_1d(ji) - rhoi * a_i_1d(ji) * h_i_1d(ji) * ( s_i_1d(ji) - zsal ) * r1_Dt_ice
                  !
               ENDIF
               !
            ENDIF
...
!!$         ENDIF
               ...
               IF( h_i_1d(ji) >= rn_sal_himin .AND. zhmelt >= 0._wp .AND. zs_brmax > sss_1d(ji) .AND. t_su_1d(ji) <= t_bo_1d(ji) ) THEN 
                  ...
                  ! ===========
                  DO jk = 1, nlay_i
                     zv_br(jk) = zmsk(jk) * sz_i_1d(ji,jk) / MAX( zs_br(jk), epsi10 )
                  ENDDO
                  ...
                  DO iter = 1, ndrainage
                     ...
                     DO jk = 1, nlay_i
                        ...
                        !
                        !!clem trick
                        zds(jk) = MAX( zds(jk), -sz_i_1d(ji,jk) + MIN( rn_simin, rn_sinew*sss_1d(ji) ) )
                        !
                        ! new salinity
                  ...
                  DO iter = 1, nflushing
                     ...
                     IF( zv_brmin >= rn_vbrc ) THEN            ! Flushing if brine volume fraction exceeds a certain treshold 
                        ...
                        DO jk = 1, nlay_i
                           ...
                           !
                           !!clem trick
                           zds(jk) = MAX( MIN( 0._wp, zds(jk) ), -sz_i_1d(ji,jk) + MIN( rn_simin, rn_sinew*sss_1d(ji) ) )
         ...
         IF( ln_drainage .OR. ln_flushing ) THEN
            DO ji = 1, npti
               DO jk = 1, nlay_i
                  zsal = sz_i_1d(ji,jk)
                  ! update salinity
                  sz_i_1d(ji,jk) = MIN( MAX( rn_simin, sz_i_1d(ji,jk) ), rn_sinew*sss_1d(ji) )
                  ! salt flux
                  sfx_res_1d(ji) = sfx_res_1d(ji) - rhoi * a_i_1d(ji) * h_i_1d(ji) * r1_nlay_i * ( sz_i_1d(ji,jk) - zsal ) * r1_Dt_ice
                  !
               END DO
            ENDDO
         ENDIF
