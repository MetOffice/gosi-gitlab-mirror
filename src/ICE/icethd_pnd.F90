MODULE icethd_pnd
   ...
   USE sbc_ice , ONLY : qsr_ice, qns_ice
   USE sbc_oce , ONLY : tprecip, sprecip
   USE icectl         ! sea-ice: control prints

   USE in_out_manager ! I/O manager
   USE lib_mpp        ! MPP library
   USE iom            , ONLY : iom_put, iom_use ! I/O manager library
   USE timing         ! Timing
   ...
   !                              !  ln_pnd_LEV
   REAL(wp) ::   rn_apnd_min      !    Minimum fraction of melt water contributing to ponds
   REAL(wp) ::   rn_apnd_max      !    Maximum fraction of melt water contributing to ponds
   REAL(wp) ::   rn_pnd_flush     !    Pond flushing efficiency (tuning parameter)
   !                              !  ln_pnd_CST
   REAL(wp) ::   rn_apnd          !    prescribed pond fraction (0<rn_apnd<1)
   REAL(wp) ::   rn_hpnd          !    prescribed pond depth    (0<rn_hpnd<1)
...
CONTAINS
   ...
   SUBROUTINE ice_thd_pnd
      ...
      INTEGER :: npti                ! Counter for number of ice points
      !!-------------------------------------------------------------------
      ...
      IF( ln_icediachk )   CALL ice_cons2D  (1, 'icethd_pnd',  diag_v,  diag_s,  diag_t,  diag_fv,  diag_fs,  diag_ft)
   ...
   SUBROUTINE pnd_CST
      ...
      DO jl = 1, jpl
         ...
         DO_2D(0, 0, 0, 0)
            !
            zdv_pnd = ( h_ip(ji,jj,jl) + h_il(ji,jj,jl) ) * a_ip(ji,jj,jl)
            !
            IF( a_i(ji,jj,jl) >= 0.01_wp .AND. t_su(ji,jj,jl) >= rt0 ) THEN
               h_ip(ji,jj,jl)      = rn_hpnd
               a_ip(ji,jj,jl)      = rn_apnd * a_i(ji,jj,jl)
               h_il(ji,jj,jl)      = 0._wp    ! no pond lids whatsoever
            ELSE
               h_ip(ji,jj,jl)      = 0._wp
               a_ip(ji,jj,jl)      = 0._wp
               h_il(ji,jj,jl)      = 0._wp
            ...
            !
            zdv_pnd = ( h_ip(ji,jj,jl) + h_il(ji,jj,jl) ) * a_ip(ji,jj,jl) - zdv_pnd
            wfx_pnd(ji,jj) = wfx_pnd(ji,jj) - zdv_pnd * rhow * r1_Dt_ice
            !
         END_2D
   ...
   SUBROUTINE pnd_LEV
      ...
      DO jl = 1, jpl
         ...
         DO_2D(0, 0, 0, 0)
            ...
            zdv_pnd = ( h_ip(ji,jj,jl) + h_il(ji,jj,jl) ) * a_ip(ji,jj,jl)
            !                                                            !----------------------------------------------------!
            IF( h_i(ji,jj,jl) < rn_himin .OR. a_i(ji,jj,jl) < 0.01_wp ) THEN   ! Case ice thickness < rn_himin or tiny ice fraction !
               !                                                         !----------------------------------------------------!
            ...
            ELSE                                                         ! Case ice thickness >= rn_himin !
               !                                                         !--------------------------------!
               v_ip(ji,jj,jl) = h_ip(ji,jj,jl) * a_ip(ji,jj,jl)   ! retrieve volume from thickness
               v_il(ji,jj,jl) = h_il(ji,jj,jl) * a_ip(ji,jj,jl)
               !
               !------------------!
               ! case ice melting !
               ...
               !    If pond area exceeds zfr_mlt * a_i(ji,jj,jl) then reduce the pond volume
               !       a_ip_max = zfr_mlt * a_i
               !       => from zaspect = h_ip / (a_ip / a_i), set v_ip_max as:
               zv_ip_max = zfr_mlt * zfr_mlt * a_i(ji,jj,jl) * zaspect
               zdv_mlt   = MAX( 0._wp, MIN( zdv_mlt, zv_ip_max - v_ip(ji,jj,jl) ) )

               ! depth driven overflow
               !    If pond depth exceeds half the ice thickness then reduce the pond volume
               !       h_ip_max = 0.5 * h_i
               !       => from zaspect = h_ip / (a_ip / a_i), set v_ip_max as:
               zv_ip_max = z1_aspect * a_i(ji,jj,jl) * 0.25 * h_i(ji,jj,jl) * h_i(ji,jj,jl)
               zdv_mlt   = MAX( 0._wp, MIN( zdv_mlt, zv_ip_max - v_ip(ji,jj,jl) ) )

               !--- Pond growing ---!
               v_ip(ji,jj,jl) = v_ip(ji,jj,jl) + zdv_mlt
               !
               !--- Lid melting ---!
               IF( ln_pnd_lids )   v_il(ji,jj,jl) = MAX( 0._wp, v_il(ji,jj,jl) - zdv_mlt ) ! must be bounded by 0
               !
               !-------------------!
               ! case ice freezing ! i.e. t_su(ji,jj,jl) < (zTp+rt0)
               !-------------------!
               !
               zdT = MAX( zTp+rt0 - t_su(ji,jj,jl), 0._wp )
               !
               !--- Pond contraction (due to refreezing) ---!
               IF( ln_pnd_lids ) THEN
                  !
                  !--- Lid growing and subsequent pond shrinking ---!
                  zdv_frz = - 0.5_wp * MAX( 0._wp, -v_il(ji,jj,jl) + & ! Flocco 2010 (eq. 5) solved implicitly as aH**2 + bH + c = 0
                     & SQRT( v_il(ji,jj,jl)*v_il(ji,jj,jl) + a_ip(ji,jj,jl)*a_ip(ji,jj,jl) * 4._wp * rcnd_i * zdT * rDt_ice / (rLfus * rhow) ) ) ! max for roundoff errors

                  ! Lid growing
                  v_il(ji,jj,jl) = MAX( 0._wp, v_il(ji,jj,jl) - zdv_frz )

                  ! Pond shrinking
                  v_ip(ji,jj,jl) = MAX( 0._wp, v_ip(ji,jj,jl) + zdv_frz )

               ELSE
                  zdv_frz = v_ip(ji,jj,jl) * ( EXP( 0.01_wp * zdT * z1_Tp ) - 1._wp )  ! Holland 2012 (eq. 6)
                  ! Pond shrinking
                  v_ip(ji,jj,jl) = MAX( 0._wp, v_ip(ji,jj,jl) + zdv_frz )
               ENDIF
               !
               ...
               a_ip(ji,jj,jl) = MIN( a_i(ji,jj,jl), SQRT( v_ip(ji,jj,jl) * z1_aspect * a_i(ji,jj,jl) ) ) ! make sure a_ip < a_i
               h_ip(ji,jj,jl) = zaspect * a_ip(ji,jj,jl) / a_i(ji,jj,jl)
               !
               ...
               zhp = ( h_i(ji,jj,jl) * ( rho0 - rhoi ) + h_ip(ji,jj,jl) * ( rho0 - rhow * a_ip(ji,jj,jl) / a_i(ji,jj,jl) ) ) * r1_rho0

               ! Calculate the permeability of the ice
               zperm = ice_perm_eff( t_i(ji,jj,:,jl), sz_i(ji,jj,:,jl) )
               
               ! Do the drainage using Darcy's law
               zdv_flush   = -zperm * rho0 * grav * zhp * rDt_ice / (zvisc * h_i(ji,jj,jl)) * a_ip(ji,jj,jl) * rn_pnd_flush ! rn_pnd_flush comes from Hunke et al. (2012)
               zdv_flush   = MAX( zdv_flush, -v_ip(ji,jj,jl) ) ! < 0
               v_ip(ji,jj,jl) = v_ip(ji,jj,jl) + zdv_flush
               ...
               IF( ln_pnd_lids ) THEN
                  ...
                  IF( a_ip(ji,jj,jl) > 0.01_wp ) THEN   ;   h_il(ji,jj,jl) = v_il(ji,jj,jl) / a_ip(ji,jj,jl)
                  ELSE                               ;   h_il(ji,jj,jl) = 0._wp
                  ENDIF
                  !--- remove ponds if lids are much larger than ponds ---!
                  IF ( h_il(ji,jj,jl) > h_ip(ji,jj,jl) * 10._wp ) THEN
                     a_ip(ji,jj,jl) = 0._wp
                     h_ip(ji,jj,jl) = 0._wp
                     h_il(ji,jj,jl) = 0._wp
                  ENDIF
               ...
               IF( ll_diag_pnd ) THEN
                  diag_dvpn_mlt(ji,jj) = diag_dvpn_mlt(ji,jj) + rhow *   zdv_avail             * r1_Dt_ice   ! > 0, surface melt input
                  diag_dvpn_rnf(ji,jj) = diag_dvpn_rnf(ji,jj) + rhow * ( zdv_mlt - zdv_avail ) * r1_Dt_ice   ! < 0, runoff
                  diag_dvpn_lid(ji,jj) = diag_dvpn_lid(ji,jj) + rhow *   zdv_frz               * r1_Dt_ice   ! < 0, shrinking
                  diag_dvpn_drn(ji,jj) = diag_dvpn_drn(ji,jj) + rhow *   zdv_flush             * r1_Dt_ice   ! < 0, drainage
               ENDIF
               !
            ENDIF
            !
...
!!$         ENDIF
   ...
   SUBROUTINE ice_thd_pnd_init
      ...
      READ_NML_CFG(numnam_ice,namthd_pnd)
